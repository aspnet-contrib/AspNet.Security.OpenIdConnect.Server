        ...
        private async Task<bool> HandleSignInAsync(AuthenticationTicket ticket) {
            ...
            if (request.IsAuthorizationRequest()) {
                ...
                if (!string.IsNullOrEmpty(request.Nonce)) {
                    ...
                    ticket.Properties.Items[OpenIdConnectConstants.Properties.Nonce] = request.Nonce;
                }

                if (!string.IsNullOrEmpty(request.RedirectUri)) {
                    // Keep the original redirect_uri parameter for later comparison.
                    ticket.Properties.Items[OpenIdConnectConstants.Properties.RedirectUri] = request.RedirectUri;
                }
            }
